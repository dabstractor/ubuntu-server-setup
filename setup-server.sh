...
configure_sudo() {
    ...
    echo "Defaults timestamp_timeout=0" | sudo tee /etc/sudoers.d/session-sudo > /dev/null
...
install_starship() {
    ...
    echo "✓ Starship installed with no-empty-icons preset"
    echo
}
...
install_znap() {
    ...
    # Initialize znap in a zsh context
    zsh -c 'source ~/.config/znap/src/znap.zsh'
    ...
    '
    echo "✓ Znap and plugins installed"
    echo
}
...
install_neovim() {
    ...
    echo "✓ neovim installed with custom config"
    echo
}
